...
| **`statecraft sync`** | Update generated rule files (Cursor, Claude, Codex) to the current template. Uses `.statecraft.json` if present; otherwise discovers and parses existing rule files. Overwrites generated content—back up if you customized. Use `--dry-run` to list files that would be updated. |
